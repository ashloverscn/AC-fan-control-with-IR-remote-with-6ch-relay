...
void setup() {
  MsTimer2::set(340, read_write_switch_state); // call analog key reader every 10ms
  MsTimer2::start();
...
void loop() {
  ...
  show_fan_speed(dim_value);//reloading and refresh display after every process
}
 /*#########################################################################################################*/ 
...
*/
void read_write_switch_state() //call this function using a timer not loop becasue loop is stuck by nec remote listner 
{
  int sw_sample1=analogswitch.sw_data(asw_pin);
  delay(1);
  int sw_sample2=analogswitch.sw_data(asw_pin);
  delay(1);
  
  if(sw_sample1 == sw_sample2)
  {
    switch(sw_sample1)
    {
      case 1: 
